...
- Sprint 6（残置機能＋自動化フロー）は **7/15 に完了**。続く 7/15〜16 で、要求仕様でスコープ外としていた**縦書きエディタ**が浮上し、SPEC策定（SPEC-vertical-editor / SPEC-vertical-editor-phase2）→ Phase 1（組版パイプライン検証）→ Phase 2（2ペインWebエディタ）の実装・本番反映まで完了した
- **Claude Fable 5 が使えるのは 7/20 まで**。また 7/17〜7/20 の4連休は仕事が休みで、まとまった作業時間をしっかり確保できる
- → 方針：**4連休はドッグフーディングより縦書きエディタの実装（Phase 3 以降）を優先**し、**エディタが終わり次第ドッグフーディング（Sprint 8）へ移行**する。8/1 運用開始・8/11 開発仕上げの枠組みはセーフティネットとして維持

### 7/18 改訂の前提（v5）

- **Claude Fable が Max プランに組み込まれた**ため、7/20 以降も同モデルで開発を継続できる（v2〜v4 の「7/20 でモデルが変わる」前提が解消）
- → 方針：**8/11 いっぱいまでをドッグフーディング期間とし、開発期間を継続**する。この期間に**バグをすべて直し切り、必要な機能追加・改修をすべて終える**ことを目標とする
- **8/12 以降は運用期間**とし、原則コードには手を入れない。要望・不具合は随時 GitHub Issue に起票して蓄積する（対応は運用期間の方針に従い原則保留）
...
### Sprint 7：縦書きエディタの実装（7/17〜7/20・4連休、Phase 3以降）

- **Phase 3: 快適性**（コメントUI・ルビ入力補助・画像アップロード/挿入補助・字数/ページ数見積り・目次/奥付の設定UI）を実装する。SPECインタビュー→詳細SPEC確定→実装の流儀を維持（SPEC-vertical-editor §5.1）
- Phase 3 完了後に余力があれば **Phase 4: ネコノテ連携**（エディタ⇄ネコノテの相互リンク、レビュー結果のコメント書き戻し等。別途SPEC策定から）に着手する
- **Fable 5 が使える 7/20 午後までに、モデル性能が効く難所を優先**して消化する（v2改訂と同じ考え方）
- ✅ 完了条件：親SPEC §10-3 のE2E項目（コメント表示・ルビ入力補助・字数カウント・画像D&D・入稿ビルド起動）を満たし、本番反映されている

### Sprint 8：実データでのドッグフーディング（縦書きエディタ完了次第〜8/11）

- 実際のCOMITIA作品の構想・企画・設計・執筆データでアプリを本格利用する（**執筆には Sprint 7 で仕上げた縦書きエディタを使い、エディタ自体もドッグフーディング対象とする**）
- **本番実機（タッチ）でのビートボードD&D確認**もここで実施（実運用を兼ねる）
- 発見した不具合・要望は **GitHub Issue に起票し、Sprint 6 で構築した自動化フローで修正を回す**
- **期間は 8/11 いっぱいまで**（v5改訂）。この期間内に**バグをすべて直し切り、必要な機能追加・改修をすべて終える**
- ✅ 完了条件：執筆プロセス一巡分（構想→企画→設計→校正）を実データで通し、既知のバグ・必要な改修が 8/11 までにすべて解消されている

### 運用期間（8/12〜）

- **原則コードには手を入れない**。アプリは執筆の道具として使うことに専念する
- 要望・不具合は随時 **GitHub Issue に起票**して蓄積する（対応は原則保留。執筆を止める重大障害のみ例外的に修正を検討）
...
- [x] **AIプロバイダの月額スペンド上限＋アラート設定**（Anthropic / OpenAI / Google の各ダッシュボード。
      監査 M-1 の対策1＝最優先。コード側のレートリミットは適用済みだが、最後の砦はこちら）
      （7/15 完了：ユーザーが3プロバイダとも設定。推奨値ベース＝Anthropic 上限$50/アラート$20・
      OpenAI 上限$20/アラート$10・Google $10相当のアラート（Gemini API はハードリミット非対応のため
      Cloud Billing 予算アラートのみ。通知で気づいて手動対処する運用）。根拠は通常利用が月$10〜25
      規模なのに対し、コード側レートリミット（chat 300回/日・review/proofread 各60回/日）下の
      最悪ケースでも被害を月$80程度で頭打ちにする設計）
...
| リスク | 兆候 | 対処 |
|---|---|---|
| ~~Sprint 6 が平日3日で終わらない~~ | ~~7/16夜時点で残置機能が未消化~~ | 解消済み（7/15 に Sprint 6 完了・自動化フロー稼働） |
| ~~縦書きエディタが 7/20 までに終わらない~~ | ~~7/20夜時点で Phase 3 が未消化~~ | 解消済み（7/17 に Phase 4 まで完了・本番反映。Fable の Max プラン組み込みで 7/20 期限自体も消滅） |
| 8/11 までにバグ・改修が消化しきれない | 8月上旬時点で未解消 Issue が積み上がっている | Issue に優先度を付け、執筆プロセスを止めるものから消化。軽微なものは運用期間の Issue 蓄積へ回し、8/11 で開発を打ち切る（期日は延ばさない） |
...
| ドッグフーディングで重大不具合が出る | 執筆プロセスが一巡できない | Issue起票→自動化フローで即修正 |
| AI応答の品質不足 | レビューが的外れ | プロンプト（レビュープロファイル）の改善を優先。モデル変更は設定で即試せる |
| 認証がまた不安定 | セッション切れ再発 | Sprint 0で検証済みのはずだが、再発したら他機能を止めてでも最優先で修正 |

**共通ルール**（キックオフドキュメントより）：同じ問題に2時間で切り替え検討、同種エラー3回で設計見直し、締切1/3経過で未着手機能はスコープカット。v5改訂により、**8/11 が開発終了の確定期日**（バグ修正・機能追加・改修をすべて終える）。**8/12 以降は運用期間**として原則コードに手を入れず、要望は GitHub Issue への起票のみ行う。
