...
### Sprint 8：実データでのドッグフーディング（縦書きエディタ完了次第〜）
...
| 縦書きエディタが 7/20 までに終わらない | 7/20夜時点で Phase 3 が未消化 | Phase 3 を項目単位でスコープカットし、未消化分は 7/21 以降の通常ペースへ回してドッグフーディングを開始する。遅れても 8/1 運用開始のセーフティネット内に収める |
...
**共通ルール**（キックオフドキュメントより）：同じ問題に2時間で切り替え検討、同種エラー3回で設計見直し、締切1/3経過で未着手機能はスコープカット。全機能が本番に揃っているため、8/1運用開始・8/11開発仕上げはセーフティネットとして維持しつつ、実質的な判断ポイントは **7/20夜**（縦書きエディタ Phase 3 の消化状況と、ドッグフーディング開始・スコープカットの判断）に置く。
