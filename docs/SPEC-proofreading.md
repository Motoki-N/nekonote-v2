...
改訂: 2026-09-22 網羅性の指示だけでは改善せず（実使用で5〜6回まわしてようやく収束）、**多段校正**へ移行（Issue #281。§2「1回の実行の周回」・§3.3・§3.5）
改訂: 2026-09-22 提案一覧の**絞り込み（すべて・未処理・処理済み）**を追加（Issue #285。§3.3）
...
| 1回の実行の周回（Issue #281・2026-09-22） | **収束まで最大6周**。1回の「校正を受ける」でサーバーが繰り返し校正し、2周目以降は既出の指摘を渡して漏れだけを拾わせる。新規指摘が0件になった周で打ち切る。作者が手で5〜6回再校正していた操作をサーバー側に畳み込む |
...
- **streamObject による逐次表示**: 確定した提案から順にカードを表示。実行中はstopボタン・多重実行抑止（既存レビューパネルと同じ作法）。
  多段化（§3.5）に伴い、各周の `elementStream` から**確定した要素単位**で書き出す（生成途中の未完カードは出さない）。
  周回の区切りはUIに出さず「校正中…（N件）」の件数だけを更新する（何周目かは作者の関心事ではない）
- **周回の結末表示**: 実行後に「3周チェックし、これ以上の指摘は出なくなりました」（収束）/
  「上限の6周までチェックしました。…もう一度校正すると追加の指摘が出る可能性があります」（上限到達）/
  「実行時間の上限により4周で打ち切りました。…」（時間切れ）/「…周目で中断しました」（エラー）を表示する。
  **出し切ったのか続きがあるのかが伝わらないと、結局また手で再校正することになる**ため必須
...
- **提案一覧の絞り込み**: 「すべて / 未処理 / 処理済み」を件数つきで切り替える（既定は**未処理**）。
  多段化で1回の指摘件数が増え、いま何に対応すればよいかが一覧に埋もれるようになったため（Issue #285）。
  **保留は未処理に含める**（判断を先送りしただけで、まだ作者の対応が要る。書き戻し忘れも防ぐ）。
  絞り込みは表示だけの都合でクライアント側に閉じ、「確定分をコミット」「保留をコメントで書き戻す」の対象は
  表示中のぶんではなく**該当する全件**のまま。実行中のストリーミング表示も絞り込みの対象外
...
- **網羅性の指示**（Issue #281）: 「再校正のたびに別の指摘が出てくる」＝1回の走査で拾い切れていない症状への対処として、
  ...
  **この指示だけでは改善しなかった**（実使用で依然5〜6回の再校正を要した）ため、下の多段校正を重ねている。
  指示自体は各周のプロンプトに残す（1周あたりの取りこぼしを減らす下地として効かせる）
- **多段校正**（Issue #281）: 1回の「校正を受ける」でサーバーが**最大6周**まで繰り返し、
  2周目以降の system に `buildAlreadyFoundGuidance`（その実行で既に挙げた指摘の一覧）を添えて
  見落とし箇所だけを拾わせる。作者が手で5〜6回再校正していた操作をサーバー側に畳み込んだもの。
  - **打ち切り条件**: ①その周の新規指摘が0件（`converged`）／②上限6周に到達（`max_passes`）／
    ③次の周を始める時点で経過210秒超（`time_limit`。`maxDuration` 300秒に食われて保存ごと失うのを避ける）／
    ④周の途中でプロバイダエラー（`error`。そこまでの指摘は活かす）
  - **重複の除去**: 周をまたいだ重複は `suggestionKey`（原文抜粋＋修正案）で落とす。
    拒否済みのキー集合（Issue #262）を初期値にするので、拒否済みの蒸し返しも同じ1つの判定で落ちる
  - **上限6周の根拠**: 実使用での収束点が5〜6回だったため。収束すれば早期に止まるので、
    多くの実行はこれより少ない周回で終わる
  - **既出一覧の件数上限は設けない**: 上限を掛けると古い指摘がすり抜けて再提案され、
    除去フィルタで黙って捨てられるだけの無駄な出力になる
  - **失敗した周の扱い**: `ai@7` の `elementStream` はエラーチャンクを捨てて正常終了するため、
    周の失敗は `streamObject` の `onError` / `onFinish(error)` でしか知れない（`for await` は投げない）。
    失敗を拾えないと「新規0件＝収束」と誤判定するので、収束判定より先に失敗を見る。
    同じ理由でトークン数も `onFinish` の `usage` から取る——`result.usage` は finish チャンクでしか
    解決されず、エラー・中断では永久に未解決のまま await が返らない（reject もされない）
  - **1周目から失敗して0件のときは保存に入らない**: `saveSuggestions` は既存 pending を削除して
    `last_reviewed_commit` を進めるため、そのまま通すと「AIが落ちただけ」で作者の未処理指摘が消える
    （多段化前の `if (!object) return;` に相当する防御）
  - **SHAを進めるのは1周でも完走したときだけ**: 各周は原稿を通しで見るので、1周完走＝全文校正が
    1回成立している。逆に1周も完走せず落ちた実行でSHAを進めると、全文を校正し切っていないのに
    更新バナーの基準だけが最新へ動く（部分校正でSHAを進めないのと同じ論理）
- **拒否済み提案の抑止**（Issue #262）: そのリンクの `status='rejected'` を全件取得し、更新日時の新しい順に最大50件を
  「作者が却下済みの指摘」節として system に添えて蒸し返しを禁じる（言い回しを変えただけの再提案も不可）。
  プロンプトだけでは取りこぼしうるため、同一性キー（`suggestionKey`＝原文抜粋＋修正案）で一致する提案を
  **保存対象から除外する**。多段化（Issue #281）以降は、拒否済みのキー集合を周またぎの重複除去に使う
  `seenKeys` の初期値に据えることで、ストリームへ流す時点で落としている（保存直前のフィルタではなくなった）。
  **プロンプトの50件上限は除外フィルタには掛けない**（古い拒否がすり抜けるため。キー集合は常に全件）。
...
- **API**: `POST /api/proofread` 新設（`/api/review` とは別ルート。文書ストリームと構造化ストリームで型が違う）。認証→ manuscript_link のRLS越し所有確認→最新原稿取得→ `resolveModel(supabase, 'low')` → `streamObject`（output: 'array'、element = 提案Zodスキーマ）を**周回**→ 全周終了後に pending 置き換え保存＋ last_reviewed_commit 更新。クライアントは `@ai-sdk/react` の `useObject` で逐次描画
- **多段化に伴う応答形式**（Issue #281）: 複数周の結果を1本にまとめ、打ち切り理由も返す必要があるため、
  `toTextStreamResponse()`（提案の配列そのもの）をやめ、自前の `ReadableStream` で
  `{"suggestions":[…],"passes":N,"stopReason":"…"}` を書き出す（`proofreadStreamSchema`＝サーバー・クライアント共用）。
  `suggestions` を先に流し切ってから `passes` / `stopReason` を書くので、ストリーミング中のクライアントには
  提案だけが順に見え、`useObject` の部分JSON解析がそのまま通る。
  保存は**ストリームを閉じる前**に済ませる（`onFinish` 相当の後処理がレスポンス終了後に走らない形にし、
  クライアントの取り直しが保存前のDBを読むレースを避ける）
