...
改訂: 2026-09-21 **一度拒否した指摘を繰り返さない**（Issue #262。§2「拒否済み提案の扱い」・§3.5）
改訂: 2026-09-22 **指摘を一度で出し切らせる網羅性の指示**を追加（Issue #281。§3.5）
...
| 拒否済み提案の扱い（Issue #262・2026-09-21） | **同じ内容の指摘は二度と出さない**。作者が拒否（このままでよい）とした提案を再校正で蒸し返さない。同一性は**原文抜粋と修正案の両方の一致**（前後の空白差は無視）で判定する。同じ箇所への明確に別観点の指摘までは塞がない |
...
- **網羅性の指示**（Issue #281）: 「再校正のたびに別の指摘が出てくる」＝1回の走査で拾い切れていない症状への対処として、
  `PROOFREAD_EXHAUSTIVE_GUIDANCE` を system に常に添える（全文校正・選択範囲校正の双方）。
  内容は「冒頭から末尾まで順に走査し原稿の登場順に出す」「自分で件数の上限を設けない」
  「同種の誤りは箇所ごとに1件ずつ挙げる（まとめない）」「末尾まで走査し終える前に切り上げない」、
  プロファイル文面の抑制的な記述（標準校正の「指摘は控えめに」）が**その観点に限った基準**であって
  件数を絞る指示ではない旨の打ち消し、および**除外指示を網羅性より優先させる但し書き**。
  但し書きは、技術書校正プロファイルの「校正対象外（コードブロック等）」・コメント内の除外（Issue #17）・
  却下済み（Issue #262）と保留済みの蒸し返し禁止を、網羅性の圧力で崩させないためのもの
  （抑制記述の打ち消しは件数にのみ効かせる）。
  出力件数が増えて実行時間が延びるため `maxDuration` は 300 秒（`/api/review` と同値）。
  プロンプトのみの対処であり、**これで不十分なら多段校正（1回の実行で自動的に複数周し、
  既出の指摘を渡して漏れを拾わせる）へ進む**——判断は実使用での検証結果を待つ
- **拒否済み提案の抑止**（Issue #262）: そのリンクの `status='rejected'` を全件取得し、更新日時の新しい順に最大50件を
  「作者が却下済みの指摘」節として system に添えて蒸し返しを禁じる（言い回しを変えただけの再提案も不可）。
  プロンプトだけでは取りこぼしうるため、`onFinish` の保存直前に同一性キー（`suggestionKey`＝原文抜粋＋修正案）で
  一致する提案を**除外してから insert** する。**プロンプトの50件上限は除外フィルタには掛けない**
  （古い拒否がすり抜けるため。キー集合は常に全件）。
  クライアントもストリーミング中のカードに同じ判定をかける。ただし生成途中で文字列が未完のカードは判定できないため、
  末尾の生成中カードが一瞬表示されることはある（確定した時点で消える）。
  判定は `lib/proofread-apply.ts` の `suggestionKey` をサーバー・クライアントで共用する。
  選択範囲校正では、選択範囲内に原文抜粋が見つかる拒否済みだけをプロンプトへ載せる
