...
- 関連ノート（同じ仮タイトルタグ＝`working_title` を持つノート群）を system プロンプトに**含める**（Issue #46・2026-07-30 改訂）
  - サーバー側でスレッドの note_id からRLS越しに取得（リクエスト形式は不変）。ごみ箱内は除外
  - トークンコスト抑制のため**更新が新しい順に最大10件**。本文は保存済みDB値（対象ノートだけがエディタ現在値）
  - プロンプト上は「参考情報」と位置づけ、掘り下げの主対象は対象ノートのままとする
