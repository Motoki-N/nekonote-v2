...
- **実施結果（2026-08-12・PR #184）**: 1,913 行 → **1,044 行**。7フック＋3コンポーネント＋共有型を抽出したところで停止した。目安の 800 行には届いていないが、残部は章オープン・保存・競合マージ・プレビューの相互参照中核で、これ以上の分割は props / ref 中継が増えて可読性が逆に下がると判断（PR に明記して合意済み）。また「1〜2抽出ごとに1PR」の想定も、依存の密さから単一PR（コミット5個で粒度を維持）へ変更した
