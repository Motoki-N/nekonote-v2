...
- **検証リポジトリ**: `Motoki-N/manuscript-poc`（実原稿「異世界デバッガー」パート1のVFM化済み・Actions実走済み）。Phase 2 のGitHub読み書き先として流用する
...
- **竜の巣プロジェクトの repo 設定を `Motoki-N/manuscript-poc`（base_path 空）へ切り替える**（設定画面からユーザーが実施）。校正・進捗も同じVFM原稿を見るようになる（`.md` は既存機能でサポート済み）。writings への本適用（.txt との同居設計）は行わない——manuscript-poc をこのまま執筆リポジトリとして育てるか、後日改めて判断する
...
| # | 内容 | 検証 |
|---|---|---|
| 1 | **技術スパイク**: VFMブラウザ変換＋Viewer自前ホスト＋Blob URL組版（manuscript-pocのテーマCSSをハードコードで注入した最小ページ） | 縦書き・判型・ノンブルが出ることを目視 |
...
各ステップで typecheck / lint。ステップ4以降で security-reviewer。E2Eはアプリ内ブラウザペインで manuscript-poc に対して実施（実コミットが発生するため、検証後は履歴に残ってよいリポジトリであることを確認済み——検証リポジトリの本来の用途）。
...
| # | 論点 | 決定 |
|---|---|---|
| A | 対象リポジトリの解決 | **既存 `projects.repo/base_path` を共有**。竜の巣の設定を manuscript-poc へ切り替える（スキーマ変更なし。校正・進捗も同じVFM原稿を見る） |
...
10. 回帰: 原稿タブ（校正）・進捗集計が manuscript-poc のVFM原稿に対して従来どおり動く
