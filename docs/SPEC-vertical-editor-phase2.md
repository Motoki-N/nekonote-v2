...
### 6.1 まとめてコミット（Issue #255・2026-09-21 追加）

コミットは履歴に残るものなので、章ごとに都度コミットするのではなく、**作業の節目**（一日の執筆を終えたとき・外出先から帰るとき）で複数章の進捗を1コミットにまとめられるようにする。

- 起点: ツールバーの「まとめて（n）」ボタン。n は未コミットの待避がある章の数（0件のときは無効）
- ダイアログ: 待避のある章の一覧（ファイル名＋待避時刻）＋章ごとのチェックボックス＋コミットメッセージ（既定 `執筆: {n}件の章を更新（ネコノテAI 縦書きエディタ）`。選択数に追従し、手で編集したら追従をやめる）＋コミット先ブランチの明示
- 対象の収集: 開いている章の待避を確定（`flushDraft` の完了を待つ）してから IndexedDB を読む。編集直後に押しても取りこぼさない
- 反映: Git Data API の**ツリー1本＋コミット1つ**（`createTree` → `createCommit` → `updateBranchRef`）。何章あっても履歴は1行。ref 更新は force しない（他所のpushと競合したら conflict）
- 楽観ロック: ファイルごとに待避の `baseSha` を HEAD のツリーの blob SHA と照合する。**1章でもずれていれば何もコミットせず** conflict を返し、どの章がずれたかを日本語で示す（部分的に反映されると、どこまで入ったかをユーザーが追えなくなる）。解消は従来どおり章単位のマージ支援（§8）
- 衝突チェックとコミットの `base_tree` は同じ HEAD に揃える（ずれていると、その間に入った他所の変更を無警告で巻き戻しうる）
- 成功後: コミットされた章の待避を削除し未保存印を消す。**開いている章が含まれていた場合は開き直して基準SHAを進める**（待避の削除を先に済ませる。残っていると §7 の自動復元で戻ってしまう）
- レート制限: 何章まとめても `editor-save` を1回だけ消費する
- 上限: 一度に50章まで
...
- 復元フロー（章を開いたとき）:
  ...
  2. 待避があり `content` がリモートと異なり `baseSha` がリモートSHAと**一致する**場合 → **自動復元**し、バナー「未保存の編集を復元しました（待避時刻）」＋[破棄する]で事後通知する（Issue #255・2026-09-21 に復元ボタン方式から変更。未コミットの章を行き来するたび復元操作を求めるのは実用上煩わしい）。破棄は待避の削除に加え**本文をリモート最新へ巻き戻す**（自動復元後は本文も戻さないと取り消しにならない）
  3. 待避の `baseSha` とリモートSHAが異なる場合（待避中に他所が更新）→ 競合フロー（§8）へ。この場合は自動復元せず、本文はリモートのまま据え置いてマージ支援に委ねる
- クリア: コミット成功時（復元バナーも閉じる）・明示破棄時。待避はキャッシュであり**正は常にGitHub**（親SPEC §6）
- **永続化要求**（Issue #255・2026-09-21）: 初回に `navigator.storage.persist()` を1回だけ呼び、待避領域を永続バケットへ昇格させる。既定の best-effort バケットはディスク逼迫時にオリジンごと退去されうるが、未コミットの本文はまだGitHubに無いため退去＝原稿の消失になる。非対応・拒否時は何もしない（ユーザーによるサイトデータ削除やWebKitの7日ルールは防げないため、**待避は消えうる前提**は変わらない。恒久対策はIssue #270 の編集中データのDB保存）
