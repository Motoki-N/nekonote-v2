...
- **状態**: 未対応（next のパッチリリース待ち。2026-07-14の対応セッションで再確認：next@16.2.10 が
  依然最新で、canary 含む全バージョンが脆弱な postcss に依存＝現時点で追従先なし）
...
全6件を1件ずつ要否検討し、修正4件・対応不可1件（パッチ待ち）・対応しない1件（監査時判断のまま）で消化した。
1指摘=1コミット。各コミットで typecheck / lint 通過、最後に本番ビルド成功を確認。

| 指摘 | 判断 | コミット | 要点 |
| --- | --- | --- | --- |
| M-1 レートリミットなし | 修正 | `2ee2c72` | `lib/rate-limit.ts` 新設（インメモリ固定ウィンドウ・ユーザー×エンドポイント単位）。chat 10回/分・300回/日、review / proofread 各3回/分・60回/日。`AppError` に `rate_limited`（429）追加 |
| L-1 repo の使用時再検証なし | 修正 | `138283d` | `repoSchema` を共有スキーマ化し、`lib/git/github.ts` で repo をURLに連結する全箇所を `validRepo()` 経由に（全GitHub API呼び出しがこのラッパーを通るため一箇所で担保） |
| L-2 models.ts に server-only なし | 修正 | `517aad6` | 1行目に `import 'server-only'` 追加 |
| L-3 サイズ上限なし | 修正 | `f8ba40d` | ノート本文10万字・タイトル500字・ペルソナ説明1万字・プロンプトテンプレート2万字・各名前100字の max 追加＋企画書/キャラクターレビューの入力組み立てに合計30万字ガード |
| L-4 postcss（next同梱） | 対応不可 | — | next@16.2.10 が最新のまま＝パッチ待ち継続。`npm audit fix --force` は next@9 ダウングレードになるため実行禁止 |
| L-5 プロンプトインジェクション残余 | 対応しない | — | 監査時の判断どおり（単一ユーザー制では受容可能。複数ユーザー化時に再評価必須） |

コード外の残作業（プロバイダのスペンド上限設定＋下記「未確認項目」のダッシュボード確認）は
**ユーザー対応の手作業として実装計画（ネコノテAI_第二期_実装計画.md §5）に転記済み**。
