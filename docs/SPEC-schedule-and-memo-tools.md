...
- **ツールの登録は dashboard 分岐のみ**。掘り下げ（note）スレッドは従来どおりツールなし＝回帰ゼロ（→ §10 で `saveMemoNote` のみ掘り下げにも導入。Issue #50）
...
12. **掘り下げのメモ化（§10）**: 掘り下げパネルで「メモにまとめて保存して」→ノート作成カード＋リンクが出て、新しいノートが作られる。保存を頼まない通常の掘り下げではツールが呼ばれない。スケジュールの確定を頼むと保存されず、相談パネルへ案内される
...
## 10. 掘り下げ（note）スレッドへのツール導入（Issue #50 追補）

§7 でスコープ外としていた掘り下げスレッドへのツール導入を実装。掘り下げ中に出た気づきを、パネルを離れずそのままメモノート化できるようにする。

- **導入するのは `saveMemoNote` のみ**。ツール定義は dashboard 分岐と共用（`buildChatTools`）で、execute・入出力スキーマは §5.1 から無変更（RLS 越しの insert＝所有境界も同一）
- **`saveSchedule` は掘り下げでは登録しない**: ノートはプロジェクトに紐づかず（手帳→浄書）、掘り下げのリクエストコンテキストに projectId が存在しないため対象を特定できない。ダッシュボードの「プロジェクト指定なし」と同じく「モデルからツール自体が見えない」扱いとし、プロンプトで「スケジュールの確定は相談パネルで」と案内させる
- **保存先は新しいノート**（掘り下げ対象ノートへの追記ではない）。掘り下げ対象への反映は既存の「ノートに挿入」ボタンが担う——ツールは「別メモへの切り出し」、挿入ボタンは「このノートへの反映」で役割が異なる
- **結果カードは相談パネルと共通コンポーネント**（`components/chats/tool-card.tsx` に抽出）。セッション内表示のみ（§3）の方針も同じ。掘り下げパネルは相談パネルのようなDB再同期での差し替えを行わないため、カード引き継ぎ（mergeToolParts）は不要

### 対象ファイル（追加分）

- `app/api/chat/route.ts`: note 分岐に `buildChatTools`（saveSchedule 無効）を登録
- `lib/ai/prompts.ts`: `buildDeepDivePrompt` にツール使用指示を追加
- `components/chats/tool-card.tsx`: ToolCard を consult-panel から抽出（共用化）
- `components/notes/deep-dive-panel.tsx`: tool part のカード表示
