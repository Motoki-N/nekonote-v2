...
| レビューが「レビューの実行に失敗しました（応答が始まる前に接続が切れました）」／「（HTTP 504…）」 | Vercel Functions の実行上限 | **アプリのエラーハンドラを経ていない＝プラットフォーム側の失敗**（アプリ由来なら固有の日本語文言が出る）。構成レビューは全シーン＋紐づけノート＋セッション履歴を送るため最初のトークンまでが最も遅く、真っ先に上限に当たる。Vercel の Functions ログで `Task timed out after N seconds` / `FUNCTION_INVOCATION_TIMEOUT` を確認し、プロジェクトの上限（`maxDuration` は現在300秒。**プランの上限までしか効かない**）と突き合わせる。あわせて同ログの `[review] phase=… promptChars=…` で入力量を確認する |
| レビューが「生成の途中で接続が切れたため、レビューを保存できませんでした」 | プロバイダまたは実行上限 | 本文が途中まで出てから切れた場合。半端なフィードバックは保存しない仕様（判定行を欠いた本文は差し戻し扱いになるため）。ログで切断の理由を確認する |
