...
| レビューが「レビューの実行に失敗しました（HTTP 504…）」 | Vercel Functions の実行上限 | **アプリのエラーハンドラを経ていない＝プラットフォーム側の失敗**（アプリ由来なら固有の日本語文言が出る）。構成レビューは全シーン＋紐づけノート＋セッション履歴を送るため最初のトークンまでが最も遅く、真っ先に上限に当たる。Vercel の Functions ログで `FUNCTION_INVOCATION_TIMEOUT` を確認し、プロジェクトの上限（`maxDuration` は**プランの上限までしか効かない**）と突き合わせる |
