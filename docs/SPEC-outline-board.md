...
改訂: 2026-08-30 章の仕組みの統一（SPEC-board-chapters。目次カードは `part='chapter'` の scenes 行のまま、新設の `kind='chapter'` を立てて小説側の章マーカーと同一の仕組みへ寄せる。行の移動・再作成はしない。描画フィルタは `part` から `kind` へ。§2 データモデル行・§4・§7 を改訂）
改訂: 2026-08-30 目次からの原稿ファイル一括生成（SPEC-manuscript-bridge。§7 スコープ外項目の昇格）
...
| データモデル | **scenes 共用**。目次カードは `part='chapter'` の1レーン。anchor・感情は null のまま使わない（draft-to-clean-model 判断基準2の範囲。新テーブルは作らない）<br>**2026-08-30 改訂**: あわせて `kind='chapter'` を立て、小説側の章マーカーと同一の仕組みに統一した（SPEC-board-chapters §3）。`part` は「どのレーンに置かれているか」、`kind` は「何のカードか」を表す |
...
- **2026-08-30 改訂**: `scenes.kind` を追加し（20260830000001）、既存の `part='chapter'` 行へ `kind='chapter'` を立てた。描画フィルタは `kind === 'chapter'` を正とする——これにより、ジャンルを小説から切り替えたプロジェクトでもビートボードで作った章がそのまま目次に並ぶ。`part='chapter'` は章マーカー専用（CHECK 制約で担保）。全件保持・描画のみフィルタ・送信は全件、という下記の整合方式は無変更
...
- 章/節の2階層化（フラット1列で開始。実需が出たらIssue起票）※ 2026-08-30 時点でも据え置き
- 章カードへのノート紐づけUI・シーンレビュー（4観点）・感情・アンカー（小説理論の項目）
- ~~目次から原稿ファイル群の一括生成~~ → SPEC-manuscript-bridge で実装（2026-08-30）／目次のエクスポート
