...
- ~~キャラクターノート1枚単位のレビュー（ノート編集画面からの実行。必要になったら別途）~~ → **Issue #47 で実装済み（§9）**
- キャラクタータグによるノート絞り込み（タグ運用に依存させない）
- ~~実行時のノート選択UI~~ → **Issue #47 で実装済み（§9）**
...
## 9. ノート単位レビュー（Issue #47 で追加）

§7 でスコープ外としていた「ノート1枚単位のレビュー」「実行時のノート選択UI」の実装。

### 9.1 設計

- **対象種別（target_phase='character'）・標準プロファイルはそのまま流用**する。「企画書一式が対象か、
  特定ノート1枚に絞ったレビューか」は `review_sessions.note_id`（nullable、FK→notes、マイグレーション
  `20260723000003_character_note_review_scope.sql`）が持つ。null = 従来通り企画書一式（完全後方互換）
- ノートは project_id を持たない（手帳→浄書モデル）ため、ノート単位レビューは必ず「どの企画書の文脈で
  レビューするか」とセットで成立する。`resolveTarget` が `proposal_notes` の紐づき（＋ごみ箱でないこと）を検証する
- running セッションの一意性は「対象×プロファイル×note_id」に拡張。「全体」と「特定ノート」のセッションは
  別系列として並存する
- AIへの入力は `buildCharacterNoteReviewInput`（lib/ai/prompts.ts）: 企画書＋対象ノート全文＋他の紐づけ
  ノートの一覧（タイトル・タグのみ。重複・矛盾確認用の周辺文脈）＋履歴。文字数ガードは企画書＋対象ノート1枚で計算
- セッション作成後に紐づけ解除・ごみ箱入りされた場合、実行時に「対象ノートが見つかりません」で安全に失敗する

### 9.2 入り口（2箇所）

- **企画書画面**: キャラクターレビューパネル（`components/projects/character-review-panel.tsx`）の
  ヘッダーに対象セレクタ。「全体（企画書＋紐づけノート全部）」（既定）と紐づけノート1枚を切り替える
- **ノート編集画面**: ツールバーに「キャラクターレビュー」ボタン（掘り下げと排他表示。
  `components/notes/note-character-review-panel.tsx`）。ノートが紐づく企画書を解決してからパネルを出す:
  0件 = 案内のみ（紐づけは企画書画面の既存導線に委ねる）／1件 = 自動選択／複数 = 選択UI

### 9.3 追加のE2E検証（§8 に加えて）

11. **企画書画面・ノート絞り込み**: 対象セレクタでノートを選んで実行 → そのノート1枚を主対象にした
    フィードバック（DB で `review_sessions.note_id` がそのノートid）。「全体」に戻すと従来のセッションに戻る
12. **セッション並存（スコープ別）**: 同一企画書・同一プロファイルで「全体」running と「特定ノート」running が
    別セッションとして共存（DB確認: `note_id` が一方 null・一方 非null）
13. **ノート編集画面**: 紐づき1件のノートで即パネルが開く／複数件で企画選択UI／未紐づけで案内のみ
14. **紐づけ解除の後始末**: ノート単位 running セッションがある状態で紐づけを解除 → 再実行で
    「対象ノートが見つかりません」の安全な失敗（クラッシュしない）
15. **RLS/認可**: 他人（架空）のノートidと自分の企画書idの組み合わせでは not_found

※ `resolveTarget` のノート紐づけ検証・`review_sessions` 所有ポリシーの note_id 条件追加・/api/review の
セッション×ノート実体の突合も **security-reviewer 必須ゲート**の対象。
