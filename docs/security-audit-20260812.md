...
- **状態**: 対応済み（2026-08-12・Step P。現ファイルはテンプレートの公開サンプル値へ差し替え、
  git 履歴は**許容**をユーザーが決定。→ 後述「修正方針」の決定結果）
...
- **本監査で判明した差分**: セッション89 の把握分に加え、**新たに1系統が判明**した
  ...
  2件に特定済み。
- **修正方針と決定結果**（2026-08-12・Step P）:
  - 現ファイルは**4箇所すべてをテンプレートの公開サンプル値へ差し替え済み**（`docs/templates/manuscript-repo/`
    が既に使っている値に統一したため、ドキュメント内の例示も一貫した）。あわせて Step P の
    最終走査で新たに見つかったインフラ識別子の実値1箇所も伏せ字化した。
  - git 履歴は3択（履歴書き換え／許容／リポジトリ作り直し）のうち**「許容」をユーザーが決定**。
    判断材料は、①author メールが全337コミットのメタデータにあり、公開リポジトリでは通常公開される
    情報であること（部分的な書き換えは成立せず、消すなら全コミット書き換えになる）②該当する私的
    リポジトリは PRIVATE のままで、名前が知られてもアクセスはできないこと ③fork 0・star 0・
    公開前のため外部クローンが存在しないこと ④書き換えると 75 PR / 108 Issue のコミットリンクと
    dev-log 内31箇所の SHA 参照が壊れ、締めくくり作業 Step 7（開発記録）の一次資料の追跡性を
    損なうこと。
  - 再発防止として、本リポジトリのローカル `user.email` を GitHub の noreply アドレスへ切り替えた
    （今後のコミットのみ適用。既存履歴のメタデータは不変）。
...
前回監査の6項目のうち、**3項目は Step P（2026-08-12）で CLI から検証して解消**した。
残る3項目は Dashboard でしか確認できないため、ユーザー確認待ちのまま Step 4（RUNBOOK）へ引き継ぐ。

> **決着（2026-08-12・Step 4）**: 残る3項目（下記 2・4・5）は「いつか一度確認する」形では風化するため、
> `docs/RUNBOOK.md` §8.2 の**四半期定期点検の項目として恒久化**した。運用期間に入る本プロジェクトでは
> こちらの方が実効性がある。あわせて Supabase の Security Advisor 実行（下記1の推奨事項）も同じ節に入れた。

1. ~~**本番SupabaseのDB実状態**~~ → **確認済み（2026-08-12・グリーン）**。マイグレーションは
   ローカル29本に対し本番も29本適用済み。`public` の全24テーブルで RLS が有効
   （Dashboard から手動作成された想定外のテーブルはなし）。Security Advisor の実行は引き続き推奨
2. **Supabase Auth設定** — リダイレクトURL許可リスト、Auth側レート制限設定（Dashboard 確認）
3. ~~**Vercelの環境変数実設定**~~ → **確認済み（2026-08-12）**。`NEXT_PUBLIC_` 名での秘密情報の
   誤登録はなし（`NEXT_PUBLIC_` は Supabase の URL と anon key のみで、いずれも公開前提の値）。
   cron 関連キーが Production 専用というスコープ方針も守られている。
   ただし**別件の不具合を発見** → 下記「Step P で発見した設定不具合」
4. **Google OAuthクライアント設定** — 承認済みリダイレクトURIの範囲（Dashboard 確認）
5. **プロバイダ側のスペンド上限設定**（前回 M-1 の対策1の実施状況。Dashboard 確認）
...
- **内容**: `lib/email.ts:10` が参照するのは `RESEND_API_KEY` だが、Vercel には**先頭の `R` が欠けた
  名前**で登録されている（Preview + Production の1エントリ）。したがって Production では
  `RESEND_API_KEY` が未定義のまま Resend クライアントが生成され、**マイルストーン締切の
  リマインドメールが送信できない状態**が続いている。
- **原因**: INC-1（`vercel env rm` によるキー消失）からの復旧時に手入力で再登録した際のタイプミス。
  cron は失敗しても 500 を返すだけで気づく導線がなく、約20日間見過ごされていた。
- **対処**: 正しい名前で登録し直し、**再デプロイ**する（Vercel は環境変数をデプロイ時に
  スナップショットするため、登録だけでは反映されない）。誤登録エントリの削除は、
  INC-1 の教訓どおりスコープ指定でもエントリ全体が消える点に注意（本件は Preview/Production とも
  不要なエントリのため実害なし）。
- **教訓**: 環境変数は「登録した」だけでは検証にならない。**アプリ側が実際に読めているか**まで
  確認する導線がなかった。RUNBOOK（Step 4）のキー再発行手順に、登録後の疎通確認を必須手順として
  織り込む。
