...
| タイトル | **自動（スレッド先頭の user 発言1行目・Markdown記号除去・最大50字）＋一覧からリネーム可**。AI生成はしない |
...
```
update chat_threads set title = coalesce(title, <今回の user 発言1行目から整形>) where id = threadId
```
...
- タイトル整形は `saveChatMessageAsNote` の1行目整形（noteTitleFrom）を共通化して使う（サーバー側で本文から生成＝クライアント注入不可）
...
3. **タイトル自動設定**: 新スレッドの先頭発言1行目がタイトルになる（Markdown記号除去・50字）。2通目以降で上書きされない
