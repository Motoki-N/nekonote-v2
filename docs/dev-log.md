...
- 残件: 目次ボードで章を並べ替えると、そのビューに映っていないシーン（ジャンル切替で章とシーンが
  混在した場合のみ発生）の所属章が変わり得る。SPEC §5.2 が「混在時のみ効く安全弁」として
  単純な `arrayMove` の一般化を指定しているため仕様どおりの挙動としたが、**Issue 化していない**。
  セッション109 の「Issue 化されていない残作業は、dev-log に書いても消える」を踏まえると、
  ここは起票するかどうかを決めておくべき箇所
