...
cpSync(
  join(root, "node_modules", "@vivliostyle", "viewer", "lib"),
  join(dest, "viewer"),
  {
    recursive: true,
  },
...
for (const pkg of ["theme-bunko", "theme-techbook", "theme-base"]) {
  cpSync(
    join(root, "node_modules", "@vivliostyle", pkg),
    join(dest, "themes", pkg),
    {
      recursive: true,
...
console.log(
  "Vivliostyle Viewer とテーマを public/vivliostyle/ へコピーしました",
);
