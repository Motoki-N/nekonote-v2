...
function parseVerdict(text: string): ReviewVerdict {
  ...
  const matches = [
    ...text.matchAll(/^\s*\**判定[:：]\s*(承認|差し戻し)\**\s*$/gm),
  ];
 ...
 */
async function fetchProposalWithNotes(
  supabase: Supabase,
  proposalId: string,
  projectId: string,
) {
  const { data: proposal, error: proposalError } = await supabase
    .from("proposals")
    .select(
      "id, project_id, writing_genre, purpose, genre, target_audience, content, status",
    )
    .eq("id", proposalId)
    .maybeSingle();
...
async function fetchNotesForScene(
  supabase: Supabase,
  sceneId: string,
): Promise<NoteContext[]> {
  const { data, error } = await supabase
  ...
  try {
    ...
    if (phase === "proposal" || phase === "character") {
      ...
      if (session.note_id !== null) {
        ...
        const noteInputChars =
          countChars(proposal.content) + countChars(target.content);
        ...
        if (phase === "proposal" && proposal.status === "draft")
          proposalIdForStatus = proposal.id;
        ...
        if (phase === "proposal") {
          ...
          if (latestVersion?.content !== proposal.content) {
            ...
            if (snapshotError)
              throw new AppError("internal", snapshotError.message);
          }
        }
      }
      ...
      const sceneNotes = await fetchSceneNotesByScene(
        supabase,
        session.project_id,
      );
      ...
      const structureTemplate = await fetchStructureTemplate(
        supabase,
        session.project_id,
      );
      prompt = buildStructureReviewInput({
        proposal,
        structureTemplate,
        scenes,
        sceneNotes,
        ...
        try {
          const { error: insertError } = await supabase
            ...
            .insert({
              ...
              content: text,
              verdict: VERDICT_PHASES.includes(phase)
                ? parseVerdict(text)
                : null,
            });
          if (insertError) {
