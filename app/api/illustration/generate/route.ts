...
export async function POST(req: Request) {
  try {
    ...
    enforceRateLimit(user.id, 'illustration-generate', { perMinute: 2, perDay: 20 })
    ...
    if (request.referenceIllustrationId) {
      ...
      if (downloadError || !blob) {
        throw new AppError('internal', `参照イラストの取得に失敗しました: ${downloadError?.message}`)
      }
    ...
    if (insertError || !row) {
      ...
      throw new AppError('internal', `イラストの保存に失敗しました: ${insertError?.message}`)
    }
