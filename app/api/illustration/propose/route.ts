...
async function fetchManuscriptMaterials(
  ...
  const credential = connected ? await patCredentialProvider.getCredential(supabase) : null
...
export async function POST(req: Request) {
  try {
    ...
    enforceRateLimit(user.id, 'illustration-propose', { perMinute: 5, perDay: 50 })

    const parsed = proposeRequestSchema.safeParse(await req.json())
    if (!parsed.success) {
      throw new AppError('validation', parsed.error.issues[0]?.message ?? 'リクエストの形式が不正です')
    }
    ...
    if (request.referenceIllustrationId) {
      const { data: reference, error: referenceError } = await supabase
        ...
        .maybeSingle()
      if (referenceError) throw new AppError('internal', referenceError.message)
      if (!reference) throw new AppError('not_found', '参照イラストが見つかりません')
    ...
    if (!persona) throw new AppError('internal', 'イラストレーターペルソナが見つかりません')
    ...
    const manuscriptFiles = await fetchManuscriptMaterials(supabase, request, project)
