...
import { getManuscriptCommitDiff, getManuscriptHistory } from "@/lib/actions/manuscripts";
 ...
 */
export function HistoryPanel({ linkId, onClose }: { linkId: string; onClose: () => void }) {
  ...
  const [diffError, setDiffError] = useState<{ sha: string; message: string } | null>(null);
  ...
  useEffect(() => {
    ...
    void getManuscriptHistory(linkId).then((result) => {
      ...
      if (!result.ok || !result.data) {
        setListError(result.ok ? "履歴の取得に失敗しました" : result.error.message);
        return;
      }
    ...
    async (sha: string) => {
      ...
      try {
        ...
        if (!result.ok || !result.data) {
          setDiffError({
            sha,
            message: result.ok ? "差分の取得に失敗しました" : result.error.message,
          });
          return;
        }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {listError && <p className="p-2 text-sm text-destructive">{listError}</p>}
        {commits !== null && commits.length === 0 && (
          <p className="p-2 text-sm text-muted-foreground">このファイルのコミットが見つかりません</p>
        )}
        {commits !== null && (
          <ul className="flex flex-col gap-1.5">
            {commits.map((commit) => {
              ...
              return (
                <li key={commit.sha} className="rounded-lg border border-border bg-card">
                  <button
                    type="button"
                  ...
                  {expanded && (
                    <div className="border-t border-border px-2.5 py-2">
                      ...
                      ) : diffError?.sha === commit.sha && !(commit.sha in patches) ? (
                        <p className="p-1 text-sm text-destructive">{diffError.message}</p>
