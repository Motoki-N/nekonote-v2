...
export function CritiquePanel({
  ...
  useEffect(() => {
    ...
    void getCritiqueBootstrap(projectId).then((result) => {
      ...
      if (result.ok && result.data) {
        ...
        setError(result.ok ? "講評設定の取得に失敗しました" : result.error.message);
      }
  ...
  const effectivePersonaId =
    selectedProfileId !== null
      ...
      : null;
  const selectedPersona = bootstrap?.personas.find((p) => p.id === effectivePersonaId) ?? null;
  ...
  const overMax = bootstrap?.totalChars != null && bootstrap.totalChars > CRITIQUE_MAX_CHARS;
  const needsConfirm =
    bootstrap?.totalChars != null && bootstrap.totalChars > CRITIQUE_CONFIRM_CHARS && !overMax;
  ...
  const runCritique = useCallback(
    async (confirmLong: boolean) => {
      ...
      const session = await createCritiqueSession(projectId, selectedProfileId, effectivePersonaId);
      if (!session.ok || !session.data) {
        setError(session.ok ? "セッションの作成に失敗しました" : session.error.message);
        return;
      }
      ...
      try {
        const res = await fetch("/api/review", {
          ...
          body: JSON.stringify({ sessionId: session.data.sessionId, confirmLong }),
          ...
          <div className="flex flex-col gap-3">
            ...
            {targetMissing && (
              <p className="rounded-lg border border-border bg-card p-3 text-sm text-card-foreground">
                ...
                <Link href={`/projects/${projectId}`} className="text-primary underline">
                  企画書タブ
                </Link>
                で設定してください。
              </p>
            ...
            {overMax && bootstrap?.totalChars != null && (
              <p className="text-sm text-destructive">
                原稿が約{bootstrap.totalChars.toLocaleString("ja-JP")}字あり、講評の上限（
                {CRITIQUE_MAX_CHARS.toLocaleString("ja-JP")}字）を超えているため実行できません。
              </p>
            )}
            {streamingText !== null && (
            ...
            {critiques.map((critique) => (
              ...
              >
                <summary className="flex cursor-pointer flex-wrap items-center gap-x-2 gap-y-0.5 p-3 text-xs text-muted-foreground [&::-webkit-details-marker]:hidden">
                  <span className="font-medium text-card-foreground">{critique.profileName}</span>
                  {critique.personaName && <span>{critique.personaName}</span>}
                  <span className="ml-auto">{dateTimeFormat.format(new Date(critique.createdAt))}</span>
                </summary>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => void runCritique(true)}>実行する</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </aside>
