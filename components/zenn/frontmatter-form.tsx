...
export function FrontmatterForm({
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto]">
      ...
      <div className="flex flex-wrap items-end gap-3 sm:col-span-2">
        <label className="flex flex-col gap-1 text-sm" htmlFor="zenn-type">
          ...
          <select
            ...
            value={value.type}
            onChange={(e) => onChange({ type: e.target.value === 'idea' ? 'idea' : 'tech' })}
          >
            <option value="tech">tech（技術記事）</option>
            <option value="idea">idea（アイデア）</option>
          </select>
        </label>
        <label className="flex min-w-48 flex-1 flex-col gap-1 text-sm" htmlFor="zenn-topics">
          トピック（カンマ区切り・5つまで）
          <Input
            id="zenn-topics"
      ...
      <p id="zenn-slug-help" className="text-xs text-muted-foreground sm:col-span-2">
        slug は記事URLとファイル名（articles/&lt;slug&gt;.md）になります。半角英小文字・数字・
        ハイフン・アンダースコアの12〜50字。{slugLocked && '投稿済みのため変更できません。'}
      </p>
    </div>
  )
}
