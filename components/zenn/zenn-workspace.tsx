...
import { publishZennArticle, saveZennArticle, uploadZennImage } from '@/lib/actions/zenn'
...
export function ZennWorkspace({ repo, initialSlug }: { repo: string; initialSlug: string }) {
  ...
  async function handleImageSelected(file: File) {
    ...
    const nameCheck = zennImageFileNameSchema.safeParse(sanitizeImageFileName(file.name))
    if (!nameCheck.success) {
      toast.error(nameCheck.error.issues[0]?.message ?? '画像ファイル名が不正です')
      return
    }
    setUploadingImage(true)
    try {
      const base64 = await fileToBase64(file)
      const result = await uploadZennImage(slugCheck.data, nameCheck.data, base64)
      if (!result.ok || !result.data) {
      ...
      setImageUrls((prev) => ({ ...prev, [markdownPath]: URL.createObjectURL(file) }))
      const view = editorViewRef.current
      if (view) {
        const pos = view.state.selection.main.from
        const stem = result.data.fileName.slice(0, result.data.fileName.lastIndexOf('.'))
