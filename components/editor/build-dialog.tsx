...
export function BuildDialog({
  ...
  useEffect(() => {
    ...
    void getBuildTagInfo(projectId).then((result) => {
      ...
      if (!result.ok || !result.data) {
        setInfoError(
          result.ok ? "タグ情報の取得に失敗しました" : result.error.message,
        );
        return;
      }
      ...
      setInfo(data);
      setTag((current) =>
        current === "" ? data[kindRef.current].suggestedTag : current,
      );
    });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      void getBuildStatus(projectId, activeTag).then((result) => {
        ...
        if (Date.now() - buildStartRef.current > POLL_TIMEOUT_MS)
          setPhase("timeout");
      });
    };
  ...
  const start = async () => {
    ...
    try {
      ...
      if (!result.ok || !result.data) {
        toast.error(
          result.ok ? "ビルドの開始に失敗しました" : result.error.message,
        );
        return;
      }
      ...
      setPhase("building");
      toast.success(
        `タグ ${result.data.tag} を作成しました。Actions がビルドを開始します`,
      );
    } finally {
      setStarting(false);
    }
  ...
  const activeArtifact = activeTag?.endsWith("-epub")
    ? KINDS.epub.artifact
    : KINDS.nyuko.artifact;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            タグを作成すると、GitHub Actions が{KINDS[kind].artifact}を生成して
            Release に添付します
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex flex-col gap-3">
            ...
            {info && info[kind].tags.length > 0 && (
              <p className="text-xs text-muted-foreground">
                これまでの{KINDS[kind].tagLabel}:{" "}
                {info[kind].tags.slice(0, 5).join(" / ")}
              </p>
            )}
            {tag.trim() !== "" && tagInvalid && (
