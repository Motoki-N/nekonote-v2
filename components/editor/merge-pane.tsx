...
export function MergePane({
  ...
  useEffect(() => {
    ...
    const shared = [
      ...
      EditorView.theme({
        '&': { fontSize: '14px', backgroundColor: 'var(--background)', color: 'var(--foreground)' },
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid shrink-0 grid-cols-2 border-b border-border text-xs text-muted-foreground">
        <span className="border-r border-border px-4 py-1.5">リモート（最新）</span>
        <span className="px-4 py-1.5">あなたの編集（マージ結果）</span>
      </div>
      <div ref={containerRef} className="min-h-0 flex-1 overflow-auto [&_.cm-mergeView]:h-full" />
