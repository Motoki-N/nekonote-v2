...
export async function resolveProfileForPhase(
  ...
  if (!data) throw new AppError('not_found', 'レビュープロファイルが見つかりません')
  if (data.target_phase !== phase) {
    throw new AppError('validation', 'このレビュー対象では使えないプロファイルです')
  }
...
export async function resolveReviewerPersona(
  ...
  if (data.persona_type !== 'reviewer') {
    throw new AppError('validation', 'レビューに起用できるのはレビュアー型のペルソナのみです')
  }
  return { id: data.id }
}
