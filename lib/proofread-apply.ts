...
export function formatWriteBackComment(item: WriteBackItem): string {
  ...
  const excerpt =
    item.original_text.length > 20
      ? `${item.original_text.slice(0, 20)}…`
      : item.original_text;
  const reason = item.reason
    ? ` 理由: ${sanitizeCommentText(item.reason)}`
    : "";
...
export function writeBackAsComments(
  ...
  for (const item of items) {
    ...
    insertions.push({
      at: lineStart,
      text: `${formatWriteBackComment(item)}\n`,
    });
  }
  ...
  for (const insertion of insertions) {
    current =
      current.slice(0, insertion.at) +
      insertion.text +
      current.slice(insertion.at);
  }
...
export function applySuggestions(
  ...
  for (const patch of patches) {
    ...
    current =
      ...
      current.slice(idx + patch.original_text.length);
  }
  return { ok: true, content: current };
}
