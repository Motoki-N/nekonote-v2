...
export const DEFAULT_ASPECT_RATIO: Record<IllustrationKind, IllustrationAspectRatio> = {
  cover: '2:3',
  insert: '2:3',
  character: '2:3',
  concept: '16:9',
...
export const illustrationRequestSchema = z
  .object({
    ...
    instructions: z.string().trim().min(1, '描いてほしい内容を入力してください').max(2000),
  ...
  .superRefine((value, ctx) => {
    if (value.kind === 'insert' && !value.manuscriptPath) {
      ctx.addIssue({ code: 'custom', message: '挿絵は対象の原稿ファイルを選択してください' })
    }
  })
...
export const illustrationProposalSchema = z.object({
  ...
  description: z.string().describe('狙いの説明。何をどう描き、なぜこの作品に合うのか（100〜200字）'),
  imagePrompt: z
    .string()
    .describe(
