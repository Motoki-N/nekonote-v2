...
export async function getZennRepo(): Promise<
  ActionResult<{ repo: string | null }>
> {
  try {
    const supabase = await createClient();
 ...
 */
export async function registerZennRepo(
  repo: string,
): Promise<ActionResult<{ repo: string }>> {
  try {
    ...
    const parsedResult = repoSchema.safeParse(
...
export async function getAiModelSettings(): Promise<
  ActionResult<AiModelSettingsData>
> {
  try {
    const supabase = await createClient();
    const [settingsResult, personasResult] = await Promise.all([
      supabase
        ...
        .select("capability, provider, model_id"),
      supabase
        .from("personas")
    ...
    if (settingsResult.error)
      throw new AppError("internal", settingsResult.error.message);
    ...
    const keyConfigured = Object.fromEntries(
      aiProviders.map((provider) => [
        provider,
        Boolean(process.env[PROVIDER_ENV_KEY[provider]]),
      ]),
...
export async function upsertAiModelSetting(
  input: AiModelSettingInput,
...
export async function deleteAiModelSetting(
  capability: ModelCapability,
): Promise<ActionResult> {
  try {
...
export async function duplicatePersona(
  ...
  try {
    ...
    const { data: source, error: sourceError } = await supabase
      .from("personas")
      .select(
        "name, description, ai_capability, reference_scope, persona_type, writing_genre",
      )
      .eq("id", pid)
      .maybeSingle();
...
export async function updatePersona(
  ...
  try {
    ...
    if (!updated || updated.length === 0) {
      ...
      if (!existing)
        throw new AppError("not_found", "ペルソナが見つかりません");
      throw new AppError(
        "validation",
        "標準ペルソナは編集できません。複製してから編集してください",
      );
    }
...
export async function listReviewProfiles(): Promise<
  ActionResult<ReviewProfileRecord[]>
> {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      ...
      .select(
        "id, name, target_phase, prompt_template, default_persona_id, is_default, writing_genre",
      )
      .order("is_default", { ascending: false })
      .order("created_at");
    if (error) throw new AppError("internal", error.message);
...
export async function duplicateReviewProfile(
  profileId: string,
): Promise<ActionResult> {
  try {
    ...
    const { data: source, error: sourceError } = await supabase
      .from("review_profiles")
      .select(
        "name, target_phase, prompt_template, default_persona_id, writing_genre",
      )
      .eq("id", pid)
      .maybeSingle();
    if (sourceError) throw new AppError("internal", sourceError.message);
    ...
    const { error } = await supabase.from("review_profiles").insert({
      ...source,
      name: `${source.name}のコピー`,
      user_id: user.id,
      is_default: false,
...
export async function getAiUsageSummary(): Promise<
  ActionResult<AiUsageSummaryRow[]>
> {
  try {
    const supabase = await createClient();
