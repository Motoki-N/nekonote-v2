...
async function loadZennContext(): Promise<ZennContext> {
  ...
  if (!data?.zenn_repo) {
    throw new AppError(
      "validation",
      "Zenn連携リポジトリが未登録です。設定から登録してください",
    );
  }
 ...
 */
export async function getZennWorkspace(): Promise<
  ActionResult<ZennWorkspaceData>
> {
  try {
    const supabase = await createClient();
    const {
...
export async function publishZennArticle(
  ...
  try {
    ...
    const { commitSha, blobSha } = await createFileContent(
      ctx.token,
      ctx.repo,
      path,
      {
    ...
    if (error instanceof AppError && error.code === "conflict") {
      return toActionError(
        new AppError(
          "conflict",
          "このslugの記事が既に存在します。slugを変更してください",
        ),
      );
    }
    return toActionError(error);
