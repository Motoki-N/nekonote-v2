...
export async function setupManuscriptRepo(
  ...
  try {
    ...
    if (!inputResult.success) {
      throw new AppError('validation', inputResult.error.issues[0]?.message ?? '入力が不正です')
    }
    ...
    if (!credential) {
      throw new AppError('validation', 'GitHubのPATが未登録です。設定画面で登録してください')
    }
    ...
    if (headSha === null) {
      ...
      const bootstrap = templateFiles.find((f) => f.path === 'README.md') ?? templateFiles[0]
      const { commitSha: bootstrapSha } = await createFileContent(token, repo, bootstrap.path, {
      ...
      const { treeSha, files: existing } = await getFullTree(token, repo, headSha)
      baseTreeSha = treeSha
      if (existing.length > 0) {
        ...
        for (const file of existing) {
          entries.push({ path: `${backupDir}/${file.path}`, mode: file.mode, type: 'blob', sha: file.sha })
