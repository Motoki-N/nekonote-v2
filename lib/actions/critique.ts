...
export async function getCritiqueBootstrap(
  ...
  try {
    ...
    const [proposalResult, profilesResult, personasResult, critiques] =
      await Promise.all([
        supabase
          .from("proposals")
...
export async function createCritiqueSession(
  ...
  try {
    ...
    const profile = await resolveProfileForPhase(
      supabase,
      profId,
      "manuscript",
    );
    ...
    if (insertError || !created) {
      throw new AppError(
        "internal",
        insertError?.message ?? "講評セッションの作成に失敗しました",
      );
    }
...
export async function writeBackCritique(
  ...
  try {
    ...
    const persona = session.personas?.name
      ...
      : "";
    const heading = sanitizeForComment(
      `[ネコノテ講評 ${date} ${session.review_profiles.name}${persona}]`,
    );
