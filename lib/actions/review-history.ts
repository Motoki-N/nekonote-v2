...
export type ReviewHistoryPhase = 'proposal' | 'structure' | 'scene' | 'character' | 'manuscript'
...
export async function listReviewSessions(
  ...
  try {
    ...
    const sceneIds = (sessions ?? [])
      .filter((s) => s.review_profiles?.target_phase === 'scene' && s.target_ref !== null)
      .map((s) => s.target_ref as string)
      .filter((ref) => uuidSchema.safeParse(ref).success)
    const noteIds = (sessions ?? []).flatMap((s) => (s.note_id !== null ? [s.note_id] : []))
    ...
    if (scenesResult.error) throw new AppError('internal', scenesResult.error.message)
    if (notesResult.error) throw new AppError('internal', notesResult.error.message)
    const sceneTitles = new Map((scenesResult.data ?? []).map((s) => [s.id, s.title]))
    const noteTitles = new Map((notesResult.data ?? []).map((n) => [n.id, n.title]))

    const summaries: ReviewSessionSummary[] = (sessions ?? []).map((session) => {
      const phase = (session.review_profiles?.target_phase ?? null) as ReviewHistoryPhase | null

      let targetLabel: string | null = null
      if (phase === 'scene' && session.target_ref !== null) {
        const title = sceneTitles.get(session.target_ref)
        targetLabel = title === undefined ? '（削除済みシーン）' : title || '（無題のシーン）'
      } else if (session.note_id !== null) {
        const title = noteTitles.get(session.note_id)
        targetLabel = title === undefined ? '（削除済みノート）' : title || '（無題のノート）'
      }
      ...
      return {
        ...
        latestVerdict: (feedbacks.at(-1)?.verdict ?? null) as ReviewVerdict | null,
      }
    })
