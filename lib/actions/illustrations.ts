...
export async function listIllustrations(
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      .from("illustrations")
      .select(
        "id, project_id, kind, title, prompt, reference_illustration_id, storage_path, created_at",
      )
...
export async function uploadReferenceImage(
  ...
  try {
    ...
    enforceRateLimit(user.id, "illustration-upload", {
      perMinute: 10,
      perDay: 100,
    });
    ...
    const title = (
      fileBase !== ""
        ? fileBase
        : `${project.title} ${ILLUSTRATION_KIND_LABEL.reference}`
    ).slice(0, 100);
    ...
    if (insertError || !row) {
      ...
      throw new AppError(
        "internal",
        `参照画像の保存に失敗しました: ${insertError?.message}`,
      );
    }
...
export async function deleteIllustration(
  ...
  try {
    ...
    if (removeError) {
      throw new AppError(
        "internal",
        `画像の削除に失敗しました: ${removeError.message}`,
      );
    }
