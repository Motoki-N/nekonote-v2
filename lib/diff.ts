...
export function splitChangedSegments(
  ...
  while (
    suffix < max - prefix &&
    d[d.length - 1 - suffix] === a[a.length - 1 - suffix]
  )
    suffix++;
...
function pairDelAdd(dels: string[], adds: string[]): DiffRow[] {
  ...
  const pairs = dels
    .slice(0, paired)
    .map((del, j) => splitChangedSegments(del, adds[j]));
...
export function parsePatch(patch: string): DiffRow[] {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("+")) {
      rows.push({
        kind: "add",
        segments: [{ text: line.slice(1), changed: true }],
      });
      i++;
      continue;
    }
...
function diffLineOps(oldLines: string[], newLines: string[]): LineOp[] {
  ...
  ops.push(
    ...oldLines.slice(n - end).map((text): LineOp => ({ op: "eq", text })),
  );
  return ops;
}
...
export function diffTexts(oldText: string, newText: string): DiffRow[] {
  ...
  for (const [k, op] of ops.entries()) {
    ...
    for (
      let t = Math.max(0, k - CONTEXT_LINES);
      t <= Math.min(ops.length - 1, k + CONTEXT_LINES);
      t++
    ) {
      visible[t] = true;
    }
