...
/**
 * 校正を一度で出し切らせるための指示（Issue #281。system プロンプト末尾に足す）。
 * 「校正を受けるたびに別の指摘が出てくる」＝1回の走査で拾い切れていない症状への対処で、
 * 走査範囲（冒頭から末尾まで）と件数を絞らない基準を明示する。
 * プロファイル文面の抑制的な記述（標準校正の「指摘は控えめに」）は観点に限った基準なので、
 * 件数そのものの抑制と解釈されないようここで打ち消す。ただし打ち消すのは件数の抑制だけで、
 * 校正対象外の範囲（技術書校正のコードブロック、コメント内）と
 * 却下済み・保留済みの蒸し返し禁止は網羅性より優先させる（最後の1行で明示）
 */
export const PROOFREAD_EXHAUSTIVE_GUIDANCE = [
  "# 網羅性",
  "作者は一度の校正ですべての指摘を受け取りたい。指摘を小出しにせず、この1回で出し切ること。",
  "- 渡されたテキストを冒頭から末尾まで順に走査し、見つけた問題をすべて挙げる。出力は原稿の登場順とする",
  "- 自分で件数の上限を設けない。「主なもの」「代表的なもの」に絞らない",
  "- 同種の誤り（同じ表記揺れ・同じ変換ミス）が複数箇所にあれば、箇所ごとに1件ずつ挙げる。まとめて1件にしない",
  "- 後半ほど見落としやすい。末尾まで走査し終える前に切り上げない",
  "- チェック観点に「指摘は控えめに」のような抑制的な記述がある場合、それはその観点（文体・表現など）に限った基準であり、件数を絞る指示ではない。誤字脱字・表記揺れ・文法誤り・記号や数字の表記統一は、機械的に判定できるものを一件残らず挙げる",
  "- ただし、校正対象外と指示された範囲（コードブロックやコマンド出力の内部、HTMLコメントの中など）と、却下済み・保留済みとして提示された指摘は、この網羅性の対象に含めない。網羅性を理由にそれらを掘り返さない",
].join("\n");
    ...
    for (const milestone of schedule.milestones) {
      const target =
        milestone.targetChars !== null
          ...
          : "目標字数なし";
      const achieved = isMilestoneAchieved(milestone, latestTotalChars)
        ? "達成済み"
        : "未達成";
      lines.push(
        `- ${milestone.dueDate}までに「${milestone.label}」（${target}・${achieved}）`,
      );
    }
  }
  return lines;
  ...
  if (schedule) {
    lines.push(
      ...
      ...savedScheduleLines(
        ...
        schedule.latest?.totalChars ?? null,
      ),
      "",
...
export function buildDeepDivePrompt({
  ...
  const lines = [
    ...
    "- ノート本文をそのまま長く引用しない",
    "",
    "# ツールの使い方（SPEC-schedule-and-memo-tools §10）",
    "- saveMemoNote は作者が「メモにまとめて」「保存して」のようにメモ化を明確に頼んだときだけ使う。勝手に保存しない（このノートとは別の新しいノートとして保存される）",
    "- 保存に成功したら、必ず短い締めの文で保存した内容を要約して返す。失敗したら失敗したことと理由を正直に伝える",
    "- 執筆スケジュールの確定保存はここではできない。頼まれたら、ダッシュボードの相談パネルでアシスタントに確定を頼むよう案内する",
    "",
    "# 対象ノート",
    `タイトル: ${note.title || "（無題）"}`,
    `タグ: ${note.tags.length > 0 ? note.tags.join("、") : "（なし）"}`,
